...
void* logging_thread(void* tid){//int K,int N,int * data_points, int ** data_points_cluster,int * cluster){
...
void*  find_cluster_data_points_thread(void * tid){//int K , int  N,int * temp_centroids,int ** cluster,int* data_points){
...
void kmeans_pthread(int num_threads,int N,int K,int* data_points,int** data_point_cluster,float** centroids,int* num_iterations){
	...
	*num_iterations = 0;
	...
	for (int i = 0; i < t; i ++) {// i on heap?
			tid[i] = i;
												
			pthread_create(&count3s_thr[i], NULL, logging_thread, &tid[i]);
	}
	for (int i = 0; i < t; i ++)
	{
		pthread_join(count3s_thr[i], NULL);
